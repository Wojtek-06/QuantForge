...
4. **Leakage foil** — point at LOB vs naive-bar MTM divergence.
5. **Walk-forward** — enable checkbox / `walk_forward.json`; stress that OOS mean matters.
6. **Risk kill** — `stress_mm.json` / tight inventory; show `risk_killed` + VaR columns.
7. **CI** — green badge / local `ctest` 50+ tests.
